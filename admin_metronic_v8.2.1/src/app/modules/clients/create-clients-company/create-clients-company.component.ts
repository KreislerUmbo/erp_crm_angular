...
export class CreateClientsCompanyComponent {
  ...
  @Output() ClientCompanyCreate: EventEmitter<any> = new EventEmitter();
  ...
  store() {
    ...
    this.clientService.registerClientPerson(data).subscribe((resp: any) => {
      ...
      if (resp.message == 403) {
        ...
        this.toast.success("Exito", "El  Cliente Company se Registró Correctamente");
        this.ClientCompanyCreate.emit(resp.client);//client del controller store
        this.modal.close();
      }
