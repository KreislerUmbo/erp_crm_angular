...
export class EditClientsPersonComponent {
  ...
  @Output() ClientPersonEdit: EventEmitter<any> = new EventEmitter();
  ...
  selectedSexo() {
    this.sexo = this.sexo == 1 ? 2 : 1;//es para el tema de sexo
  }
  ...
  store() {
    ...
    this.clientService.updateClientPerson(this.CLIENT_SELECTED.id,data).subscribe((resp: any) => {
      console.log(resp);
      if (resp.message == 403) {
        this.toast.error("Validacion", resp.message_text);
      } else {
        this.toast.success("Exito", "El  Cliente Persona se Modificó Correctamente");
        this.ClientPersonEdit.emit(resp.client);//client del controller store
        this.modal.close();
      }
